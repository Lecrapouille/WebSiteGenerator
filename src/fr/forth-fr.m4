...
LISTE(ROUGE(En rouge) les littéraux;,VERT(En vert) la définition d'un
...
espaces. L'interpréteur Forth va parser un script (donné sur le
...
caractères ASCII (historiquement la taille maximale d'un mot Forth
est de 31 caractères ASCII mais cette limitation a disparue sur
...
AST intermédiaires contenant des pseudo-instructions de plus en plus
proche de l'assembleur de la machine cible. Le compilateur aurait
également généré un graphe colorié afin de connaître le nombre minimal
de variables nécessaires, etc.</p>
...
les nombres qu'il rencontre et exécute les opérandes qui vont piocher
les nombres dans la pile, comme nous allons le voir dans la section
suivante.</p>
...
langages mal adaptés à la récursion terminale tels que le C).</p>
...
LISTE(le LFA du dernier mot inséré dans le dictionnaire. C'est le mot
...
<p>En mode EXECUTION :</p> LISTE(Si le mot lu est connu du
